...
class Calibration(object):
    ...
    def hom_to_cart(self, pts_hom):
        """
        My implementation
        """
        """
        :param pts_hom: (N, 4 or 3)
        :return pts: (N, 3 or 2)
        """
        pts = pts_hom[:,:-1]
        return pts
    ...
    def rect_to_lidar(self, pts_rect):
        ...
        My implementation!
        """
        # print("V2C",self.V2C.T.shape)
        # print("R0",self.R0.T.shape)
        # print("dot",np.dot(self.V2C.T, self.R0.T).shape)
        pts_lidar_hom = np.dot(pts_rect, LA.pinv(np.dot(self.V2C.T, self.R0.T)))
        pts_lidar = self.hom_to_cart(pts_lidar_hom)
        return pts_lidar
